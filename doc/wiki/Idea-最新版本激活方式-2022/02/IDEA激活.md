>
> 已更新至2023.2.X 版本. 2023.2.1版本确认可激活
>


1.在官网下载安装最新的idea（此破解网站目前只有最新版本的破解补丁，未尝试过低版本，新激活方式应该可以激活旧版本）

2.打开网站 https://jetbra.in/s   或 直接访问 https://3.jetbra.in/ （idea 开发公司名称为jetbrains ） 可能需要翻墙,稳定地址已调整至暗网



3.下载 ja-netfilter-all.zip      [点击下载文件](../../uploads/f515e97ebaa0a0f81bd6766941ba6ba8/jetbra.zip)

![image](../../uploads/8f977ee86aeef7904152a5b1e47bc73d/image.png)

4.解压ja-netfilter-all.zip ,并将解压出的文件放置在固定位置,如idea 安装目录下

5.运行解压文件中的ja-netfilter-all\scripts 下的 install-all-users.vbs 或   install-current-user.vbs 文件, 点击确认等待1-2分钟左右 出现done的弹窗.破解完成

6. 打开idea 输入激活码或输入激活网站  下列激活码为已更新至2023.2.X版本

`
29VRVXKXEQ---
`



7.其他信息可阅读README


7.信息来源  idea破解补丁及无限使用补丁作者 知了(主页:https://zhile.io)

https://zhile.io/2021/11/29/ja-netfilter-javaagent-lib.html

https://jetbra.in/s